...
class RequestHandler(object):
    ...
    def __init__(self, application, request, **kwargs):
        ...
        self._headers_written = False  # flush() 一个标志位，就是buffer里面的数据是不是发送到network
        self._finished = False  # finish() 一个标志位，如果request成功返回之后，设置为True，其他异步的函数根据这个判断是否已经返回
    ...
    def head(self, *args, **kwargs):  # 不允许使用该方法
        raise HTTPError(405)
    ...
    def on_finish(self):  # 请求回复给客户端之后执行
    ...
    def clear(self):  # 清空和设置self._headers
        ...
        self._write_buffer = []  # ?
        self._status_code = 200  # 状态码
        self._reason = httputil.responses[200]  # ？

    def set_default_headers(self):  # 覆盖clear设置的self._header元素
    ...
    def set_status(self, status_code, reason=None):  # reason就是对状态码的描述
    ...
    def get_status(self):  # 获取状态码
        """Returns the status code for our response."""
        return self._status_code

    def set_header(self, name, value):  # 设置http头的属性，基本就是一一对应
    ...
    def add_header(self, name, value):  # 和set_header有点不同，主要是多值的时候使用
    ...
    def clear_header(self, name):  # 清空某一个单值属性
    ...
    _INVALID_HEADER_CHAR_RE = re.compile(br"[\x00-\x1f]")  # bytes_type不允许的字符

    def _convert_header_value(self, value):  # 装换一个值为header可以接受的格式
        if isinstance(value, bytes_type):
            pass
        elif isinstance(value, unicode_type):
            value = value.encode('utf-8')
        elif isinstance(value, numbers.Integral):  # 整数类型
    ...
    def get_argument(self, name, default=_ARG_DEFAULT, strip=True):
        # name就是arg的名字， default就是默认的返回列表，strip是不是去掉最后的空格
        # 返回一个值，就是匹配的list的最后一个元素
    ...
    def get_body_argument(self, name, default=_ARG_DEFAULT, strip=True):
        # self.request.arguments 包含了url和body里面的参数
        # self.request.body_arguments 只是包含了body里面的参数，通常是post参数
        """Returns the value of the argument with the given name
    ...
    def get_query_argument(self, name, default=_ARG_DEFAULT, strip=True):
        # self.request.query_arguments 这个是url里面的参数
        """Returns the value of the argument with the given name
    ...
    def _get_argument(self, name, default, source, strip=True):
        # source就是self.request.arguments
        # 多值默认返回最后一个
    ...
    def _get_arguments(self, name, source, strip=True):
        # 返回一个值的列表
        values = []
        for v in source.get(name, []):  # 对互获取的value_list进行遍历
            ...
            if strip:
                v = v.strip()
            values.append(v)
        return values  # 返回一个列表

    def decode_argument(self, value, name=None):  # unicode None不变 其他的转换为utf-8
    ...
    def set_cookie(self, name, value, domain=None, expires=None, path="/",
        ...
        # domain 指定哪个域名的时候需要发送，默认是当前
        # expires expires_days ?
        # path 指定特殊的路径才发送cookie到服务器
    ...
    def create_signed_value(self, name, value, version=None):  # 创建加密的cookie
    ...
    def redirect(self, url, permanent=False, status=None):  # 不明白
    ...
    def write(self, chunk):  # 添加元素到self._write_buffer
    ...
    def require_setting(self, name, feature="this feature"):  # 判定具有一个设置
...
def create_signed_value(secret, name, value, version=None, clock=None):  # 加密一个cookie
    if version is None:
...
def decode_signed_value(secret, name, value, max_age_days=31, clock=None,min_version=None):  # 解密一个加密的cookie
    if clock is None:
        clock = time.time
