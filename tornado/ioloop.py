...
# 多路复用的函数会等待返回值，如果过了_POLL_TIMEOUT秒，
# 还是没有连接进来，这里只是返回一个空的列表
_POLL_TIMEOUT = 3600.0


class TimeoutError(Exception):  # 超时错误
    pass
...
class IOLoop(Configurable):
    ...
    _EPOLLIN = 0x001  # 文件描述符可读，有东西进来了
    _EPOLLPRI = 0x002  # 有紧急的数据可读 primary
    _EPOLLOUT = 0x004  # 文件描述符可写
    _EPOLLERR = 0x008  # 文件描述符错误
    _EPOLLHUP = 0x010  # 文件描述符被挂断
    _EPOLLRDHUP = 0x2000  # 异常断开
    ...
    # 在采用这两种模式时要注意的是,如果采用ET模式,那么仅当状态发生变化时才会通知,
    ...
    READ = _EPOLLIN  # 可读
    WRITE = _EPOLLOUT  # 可写
    ERROR = _EPOLLERR | _EPOLLHUP  # 错误

    # Global lock for creating global IOLoop instance
    _instance_lock = threading.Lock()  # 一个全局的锁，各个线程之间相互制约的变量
    ...
    @staticmethod
    def initialized():  # 判断是否被初始化了，是否具有_instance变量
    ...
    # 将当前的self作为当前线程的一个局部变量IOLoop._current.instance = self
    def make_current(self):  # 将self作为一个线程局部的变量
    ...
    @classmethod
    def configurable_base(cls):  # 返回一个自己的类
        return IOLoop
...
pp        except from a signal handler.  Note that this is the **only**
