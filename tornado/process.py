...
def fork_processes(num_processes, max_restarts=100):
    ...
    if ioloop.IOLoop.initialized():  # 如果IOLoop已经初始化了，无法使用多进程的方案
    ...
    def start_child(i):
        ...
        if pid == 0:
            ...
            _reseed_random()
            global _task_id
            _task_id = i  # 设置一个全局的变量
            return i
        else:
            children[pid] = i  # 在父进程里面记录children
            return None

    for i in range(num_processes):
        id = start_child(i)
        if id is not None:
            return id  # 返回子进程的ID，子进程运行到这里就OK了
    ...
    while children:
        try:
            ...
            if errno_from_exception(e) == errno.EINTR:  # 被中断的系统调用，可以恢复的错误
                continue
            raise  # 这里不理解啊
        if pid not in children:
            continue  # 或许是其他的进程，这里是防御性质的东西
        id = children.pop(pid)
        ...
        new_id = start_child(id)  # 重新开启一个进程
        if new_id is not None:
            return new_id  # 返回子进程的ID
